...
uses SysUtils, Types, Classes, json, Variants, jsonVariant;
...
begin
{$IFDEF USE_LOGGER}
  Logger.InfoFmt('Starting test "%s" ...', [Caption]);
{$ENDIF}
  try
    proc();
{$IFDEF USE_LOGGER}
    Logger.InfoFmt('Test "%s" completed without errors.', [Caption]);
{$ENDIF}
  except
{$IFDEF USE_LOGGER}
    on E:Exception do
      Logger.ErrorFmt('Exception (%s) raised: %s', [E.Classname, E.Message]);
{$ENDIF}
  end;
end;
